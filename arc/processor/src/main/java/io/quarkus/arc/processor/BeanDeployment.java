...
public class BeanDeployment {
        ...
        for (BeanInfo bean : beans) {
            bean.types.stream().map(Type::name).distinct().forEach(rawTypeName -> {
                ...
                if (beans == null) {
                    ...
                    map.put(rawTypeName, ImmutableList.of(bean));
                } else {
                    if (beans.size() == 1) {
                        map.put(rawTypeName, ImmutableList.of(beans.get(0), bean));
                    } else {
                        ...
                        map.put(rawTypeName, ImmutableList.copyOf(array));
                    }
                }
            });
        }
    ...
    Collection<BeanInfo> getBeansByRawType(DotName typeName) {
    	Collection<BeanInfo> ret = beansByType.get(typeName);
        if (ret == null) {
