...
public class ArcContainerImpl implements ArcContainer {
    ...
    public ArcContainerImpl(CurrentContextFactory currentContextFactory) {
        ...
        this.beans = ImmutableList.copyOf(beans);
        this.interceptors = ImmutableList.copyOf(interceptors);
        this.decorators = ImmutableList.copyOf(decorators);
        this.observers = ImmutableList.copyOf(observers);
        this.removedBeans = ImmutableList.copyOf(removedBeans);
        this.transitiveInterceptorBindings = ImmutableMap.copyOf(transitiveInterceptorBindings);
        this.qualifierNonbindingMembers = ImmutableMap.copyOf(qualifierNonbindingMembers);
    }
    ...
    private void putContext(InjectableContext context, Map<Class<? extends Annotation>, List<InjectableContext>> contexts) {
        ...
        if (values == null) {
            ...
            multi.addAll(values);
            multi.add(context);
            contexts.put(context.getScope(), ImmutableList.copyOf(multi));
        }
    }
    ...
    public void init() {
        ...
        Set<Annotation> qualifiers = ImmutableSet.of(Initialized.Literal.APPLICATION, Any.Literal.INSTANCE);
    ...
    public <T> List<InstanceHandle<T>> listAll(Class<T> type, Annotation... qualifiers) {
        return Instances.listOfHandles(CurrentInjectionPointProvider.EMPTY_SUPPLIER, type, ImmutableSet.copyOf(qualifiers),
                new CreationalContextImpl<>(null));
    }

    @Override
    public <T> List<InstanceHandle<T>> listAll(TypeLiteral<T> type, Annotation... qualifiers) {
        return Instances.listOfHandles(CurrentInjectionPointProvider.EMPTY_SUPPLIER, type.getType(), ImmutableSet.copyOf(qualifiers),
                new CreationalContextImpl<>(null));
    }
    ...
    Set<Bean<?>> getBeans(Type requiredType, Annotation... qualifiers) {
        ...
        return ImmutableSet.copyOf(getMatchingBeans(new Resolvable(requiredType, qualifiers)).toArray(new Bean<?>[] {}));
    }
    ...
    private static Set<InjectableBean<?>> resolve(List<InjectableBean<?>> matching) {
        if (matching.isEmpty()) {
            ...
            return ImmutableSet.of(matching.get(0));
        }
        ...
        if (nonDefault.isEmpty()) {
            ...
            return ImmutableSet.copyOf(matching);
        } else if (nonDefault.size() == 1) {
            return ImmutableSet.of(nonDefault.get(0));
        }
        ...
        if (priorityBeans.isEmpty()) {
            ...
            return ImmutableSet.copyOf(nonDefault);
        } else if (priorityBeans.size() == 1) {
            return ImmutableSet.of(priorityBeans.get(0));
        } else {
            ...
            return ImmutableSet.copyOf(priorityBeans);
        }
    }
...
//                StackWalker walker = StackWalker.getInstance();
                List<StackTraceElement> stackTraces = Arrays.asList(Thread.currentThread().getStackTrace());
                StackTraceElement frame = this.findCaller(stackTraces);
                int idx = frame != null ? stackTraces.indexOf(frame) : -1;
    ...
    private StackTraceElement findCaller(List<StackTraceElement> traces) {
        return traces.stream()
                .filter(this::isCallerFrame)
                .findFirst().orElse(null);
    }

    private String collectStack(List<StackTraceElement> traces, int startIdx) {
        return (startIdx > 0 ? traces.subList(startIdx, traces.size()) : traces).stream()
                .map(Object::toString)
                .collect(Collectors.joining("\n\t"));
    }

    private boolean isCallerFrame(StackTraceElement frame) {
    ...
    List<Decorator<?>> resolveDecorators(Set<Type> types, Annotation... qualifiers) {
        ...
        for (InjectableDecorator<?> decorator : this.decorators) {
            if (decoratorMatches(types, ImmutableSet.copyOf(qualifiers), decorator.getDelegateType(),
